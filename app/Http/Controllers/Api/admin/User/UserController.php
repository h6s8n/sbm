...
class UserController extends Controller
{
    ...
    public function datagrid(){
        ...
        $order_by = $this->request->get('order_by' , 'users.created_at');
        ...
        $users = User::where($where_array)->where('approve', '2')
                ->where(function ($query) use ($filter_name) {
                    $query->search($filter_name, false);
                })
            ->select(
                'id','mobile','fullname','email','status','credit'
            )
            ...
            ->when($search,function ($query) use ($search){
                ...
                $query->where('users.fullname' , 'LIKE' , '%'. $search .'%')
                    ...
                    ->orWhere('users.family' , 'LIKE' , '%'. $search .'%')
    ...
    {
        $user = User::with(['city','state'])->findOrFail($id);
    ...
    public function store(){
        ...
        [
            ...
            'email.required_without' => 'ایمیل الزامی است',
            'email.unique' => 'ایمیل تکراری است',
            'mobile.required_without' => 'موبایل الزامی است',
            ...
            'birthday.required'=>'تاریخخ تولد الزامی است',
