...
@section('page_name', ' صورت حساب مالی بیمارستان ها ')
    ...
    <div class="white-box">
        <div class="portlet-body">
            ...
            <form id="myForm" class="filter_list" method="get" style="direction: rtl">
                ...
                <div class="col-md-3 col-xs-12">
                    <div class="form-group">
                        <label for="filter_partner">بیمارستان</label>
                        <select id="filter_partner" class="form-control" name="filter_partner">
                            <option >لطفا انتخاب کنید</option>
                            @foreach($partners as $partner)
                                <option value="{{$partner->id}}" {{ (@$_GET['filter_partner'] == $partner->id) ? 'selected' : '' }}>
                                    {{$partner->name}}
                                </option>
                            @endforeach
                        </select>
                ...
                @if(isset($request['data']) && count($request['data']) > 0)
                    ...
                    <ul class="pagination" role="navigation">
                        <li class="page-item" aria-disabled="true" aria-label="&laquo; Previous">
                            <a @if($request['prev_page_url']) href="{{request()->fullUrl()}}&{{$request['prev_page_url']}}" class="page-link" @else class="disable" @endif rel="prev" aria-label="&lsaquo; Previous">&lsaquo;</a>
                        </li>

                        <li class="page-item">
                            <a @if($request['next_page_url']) class="page-link" href="{{request()->fullUrl()}}&{{$request['next_page_url']}}" @else class="dissable" @endif rel="next" aria-label="Next &raquo;">&rsaquo;</a>
                        </li>
                    </ul>
                    <div class="clearfix"></div>
