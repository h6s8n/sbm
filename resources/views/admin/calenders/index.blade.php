            ...
            <div class="row">
                ...
                <div class="col-sm-2 col-xs-12">
                    @if(isset($_GET['partner']) && !empty($_GET['partner']) && !empty($_GET['doctor']))
                        <a href="{{ url('cp-manager/calender/add?partner=' . $_GET['partner'] . '&user_id='. $_GET['doctor']) }}"
                           class="btn btn-block btn-info btn-rounded">افزودن برنامه جدید</a>
                    @else
                        <a href="{{ url('cp-manager/calender/add') }}" class="btn btn-block btn-info btn-rounded">افزودن برنامه جدید</a>
                    @endif
                </div>
                <div class="col-sm-2 col-xs-12">
                        <a href="{{route('pattern.index')}}"
                           class="btn btn-block btn-info btn-rounded">الگوهای ثبت شده</a>
                </div>
...
{{--                    {{dd(\Carbon\Carbon::getDays())}}--}}
                    @foreach($request as $k => $item)
                        <tr role="row" class="filter">
                            ...
                            <td>
                                <div class="col-xs-12">
                                    <a class="btn btn-block btn-danger btn-rounded request_but" href="{{ url('cp-manager/calender/delete/' . $item['id']) }}"> حذف </a>
                                </div>
                            </td>
                        </tr>
                    @endforeach
