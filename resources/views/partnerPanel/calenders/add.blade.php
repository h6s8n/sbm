...
@if($partner)
    @section('page_name', 'افزودن برنامه برای ')
@else
    ...
    <div class="white-box">
        ...
        <form method="post" action="" enctype="multipart/form-data" class="avatar" style="direction: rtl">
            ...
            <div class="form-group row">
                ...
                <div class="col-md-6">
                    <select id="doctor" class="js-example-basic-multiple form-control{{ $errors->has('doctor') ? ' is-invalid' : '' }}" name="doctor">
                        @foreach($doctors as $doctor)
                            <option value="{{ $doctor['id'] }}" {{ (old('doctor') == $doctor['id']) ? 'selected' : '' }}>{{ $doctor['fullname'] }}</option>
                        @endforeach
                    </select>
            ...
            <div class="form-group row">
                ...
                <div class="col-md-6" style="display: flex;justify-content: space-between;">
                    <label>
                        <input type="radio" class="form-control" name="type" id="type" value="1" checked>ویزیت معمولی</label>
                    <label>
            ...
            <div class="form-group row">
                ...
                <div class="col-md-6">
                    <input id="price" type="number" class="form-control{{ $errors->has('price') ? ' is-invalid' : '' }}" name="price" value="{{ old('price', 0) }}" min="0">
            ...
            <div class="form-group row">
                <label for="day" class="col-md-4 col-form-label text-md-right"> تاریخ شروع ویزیت (شمسی)</label>

                <div class="col-md-2">
                    <select id="day" class="form-control{{ $errors->has('day') ? ' is-invalid' : '' }}" name="day">
                        <option value="">روز</option>
                        @for($d = 1 ; $d <= 31; $d++)
                            <option value="{{ $d }}" {{ (old('day', jdate('d')) == $d) ? 'selected' : '' }}>{{ $d }}</option>
                        @endfor
                    </select>
                ...
                <div class="col-md-2">
                    <select id="month" class="form-control{{ $errors->has('month') ? ' is-invalid' : '' }}" name="month">
                        ...
                        @foreach($MonthList as $key => $day)
                            <option value="{{ $key }}" {{ (old('month', jdate('m')) == $key) ? 'selected' : '' }}>{{ $day }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-2">
                    <select id="year" class="form-control{{ $errors->has('year') ? ' is-invalid' : '' }}" name="year">
                        ...
                        @foreach($YearList as $key => $year)
                            <option value="{{ $key }}" {{ (old('year', jdate('Y')) == $key) ? 'selected' : '' }}>{{ $year }}</option>
                        @endforeach
                    </select>
                    @if ($errors->has('year'))
                        <span class="invalid-feedback">
            ...
            <div class="form-group row">
                ...
                <div class="col-md-6">
                    <select id="sum_date" class="form-control{{ $errors->has('sum_date') ? ' is-invalid' : '' }}" name="sum_date">
                        @for($d = 1 ; $d <= 30; $d++)
                            <option value="{{ $d }}" {{ (old('sum_date') == $d) ? 'selected' : '' }}>{{ $d }} روز</option>
                        @endfor
                        @for($d = 2 ; $d <= 12; $d++)
                            <option value="{{ $d * 30 }}" {{ (old('sum_date') == $d * 30) ? 'selected' : '' }}>{{ $d}} ماه</option>
                        @endfor
                    </select>
            ...
            <div class="form-group row">
                ...
                <div class="col-md-6">
                    ...
                    @endphp
                    <select id="date_time" multiple class="form-control{{ $errors->has('date_time') ? ' is-invalid' : '' }}" name="date_time[]">
                        @foreach($dateTimeList as $day)
                            <option value="{{ $day }}" {{ (old('date_time') == $day) ? 'selected' : '' }}>{{ $day }}</option>
                        @endforeach
                    </select>
            ...
            <div class="form-group row">
                ...
                <div class="col-md-6">
                    <select id="time" multiple class="form-control{{ $errors->has('time') ? ' is-invalid' : '' }}" name="time[]">
                        <option value="24" {{ (old('time') == 24) ? 'selected' : '' }}> 00 {{ ' الی ' . 1 }}</option>
                        @for($d = 1 ; $d <= 23; $d++)
                            <option value="{{ $d }}" {{ (old('time') == $d) ? 'selected' : '' }}>{{ $d . ' الی ' . ($d + 1) }}</option>
                        @endfor
                    </select>
            ...
            <div class="form-group row">
                ...
                <div class="col-md-6">
                    <select id="capacity" class="form-control{{ $errors->has('capacity') ? ' is-invalid' : '' }}" name="capacity">
                        @for($d = 1 ; $d <= 10; $d++)
                            <option value="{{ $d }}" {{ (old('time') == $d) ? 'selected' : '' }}>{{ $d . ' نفر' }}</option>
                        @endfor
                    </select>
